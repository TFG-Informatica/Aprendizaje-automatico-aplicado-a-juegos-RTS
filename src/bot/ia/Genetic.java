...
public class Genetic {
	...
	private final int TOURNSIZE = 2;
	private final double MUT_CHANCE = 0.3;
	private final int MUT_TIMES = 3;
	private final int MAX_CYCLES = 8000;
	private final int REQ_DIFF_PH = 4;
	...
	private int phases;
	private int req_diff;

	public Genetic(int a_popSize, int a_bestSize, int a_phases, UnitTypeTable a_utt, 
		...
		OUT = new PrintStream(new FileOutputStream("data/GeneticSelf64x64.csv"));
	}
		...
		for (int i = 0; i < population.size(); ++i) {
			...
			for (int j = 0; j < scripts.size(); ++j) {
				...
				scripts.set(j,new GeneralScript(scripts.get(j).getUtt(),
						population.get(i).getTimeBudget(), population.get(i).getIterationsBudget(),
